...
import online.nexalink.app.AppConfig.LOOPBACK
...
object HttpUtil {
    ...
    fun getUrlContentWithUserAgent(request: UrlContentRequest): String {
        ...
        while (redirects++ < maxRedirects) {
            ...
            // forcedHeaders перекрывают одноимённые из requestHeaders (например HWID
            // должен всегда доезжать до сервера, даже если у подписки уже задан
            // другой набор кастомных заголовков).
            request.forcedHeaders?.forEach { (key, value) ->
                try {
                    requestBuilder.header(key, value)
                } catch (_: IllegalArgumentException) {
                }
